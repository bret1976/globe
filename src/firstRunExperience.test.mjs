...
test('a shorts pack hash is a share link and suppresses the first-run launcher', () => {
  assert.equal(
    shouldShowFirstRun({
      ...fresh(),
      location: { hash: '#shorts=traffic', search: '' },
    }),
    false,
  );
  assert.equal(
    shouldShowFirstRun({
      ...fresh(),
      location: { hash: '', search: '?shorts=cctv' },
    }),
    false,
  );
});

test('an active shorts pack still suppresses first-run after share-state rewrite', () => {
  const originalDocument = globalThis.document;
  globalThis.document = { body: { dataset: { shortsPack: 'traffic' } } };
  try {
    assert.equal(
      shouldShowFirstRun({
        ...fresh(),
        location: { hash: '#v=2&lat=30.27', search: '' },
      }),
      false,
    );
  } finally {
...
test('the voice TOOL SCHEMA matches the pinned release — the mission mapping is instructions only', () => {
  ...
  assert.equal(block.length, 26292, 'serialized tool schema length drifted');
  assert.equal(
    crypto.createHash('sha256').update(block).digest('hex'),
    '7ee62ae0b85a1dadea9de00422f2fe0814a74c0b60534b19f220135cebdb070a',
